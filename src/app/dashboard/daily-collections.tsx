...
import { formatAmount, formatDate, formatDateShort } from "@/lib/receipt-utils";
            ...
            >
              {days.map((d, index) => {
                ...
                return (
                  ...
                  >
                    {/* DEMO BUILD — each bar grows from the axis, one after the
                      next along the month. `originBottom` matters: scaling a
                      bar about its centre makes it sink through the baseline on
                      the way up. */}
                    <motion.div
                      className="relative w-full origin-bottom overflow-hidden rounded-t-md"
                      style={{ height: `${Math.max(h, 1.5)}%` }}
                      initial={{ scaleY: 0, opacity: 0 }}
                      animate={{ scaleY: 1, opacity: 1 }}
                      transition={{
                        ...BOUNCY,
                        // Capped, or a 400-day range would still be drawing
                        // itself a minute after the page loaded.
                    ...
                    >
                      ...
                      {Number(d.cash) > 0 ? (
                        <div
                          className={
                            Number(d.upi) > 0 ? "w-full" : "w-full rounded-t"
                          }
                          style={{
      ...
      <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-xs">
        ...
        {busiest ? (
          <span className="ml-auto flex items-center gap-1.5 text-muted-foreground">
            ...
            {t("chart.busiestDay")}:{" "}
            {formatDate(busiest.collection_date, locale)} ·{" "}
